#%%
print("hello world")
ISSN = True # image show true or not
...
def Matern_32(H, eta = 10, sigma = 1.0):
    H = H / np.amax(H) # array max, only return one value instead of many max of each columns or rows
...
def meshgrid(X, Y, Z):
    ...
    for i in range(d):
        x[i, :, :] = X
        y[i, :, :] = Y
        z[i, :, :] = Z[i] * np.ones_like(X)

    return x, y, z
    print("meshgrid is done")

# ==================================== Main Function ========================================================
# Set up the 3D grid

# Z, X, Y = np.mgrid[-20:20:10j, -100:100:100j, -100:100:100j]
dx = 50
dy = dx * np.sin(np.pi/3) # gap between grid points to discretise the regions
dz = 2.5 # depth variation
# redefine the grid
X = np.arange(0, 1000, dx).reshape(-1, 1)
Y = np.arange(0, 1000, dy).reshape(-1, 1)
Z = np.arange(0, 20, dz).reshape(-1, 1)
d = Z.shape[0]
x, y = np.meshgrid(X, Y)
y = np.flipud(y)
x[::2, :] = x[::2, :] + dx / 2

# T = np.zeros([Z.shape[0], Y.shape[0], X.shape[0]])
# for i in range(T.shape[0]):
#     T[i, :, :] = Z[i] * np.ones_like(x)

xx, yy, zz = meshgrid(x, y, Z)

def f(x, y, z):
    layers, m, n = x.shape
    val = np.zeros([layers, m, n])
    for i in range(layers):
        val[i, :, :] = 1.5 * np.cos(15 * x[i, :, :]) + 0.5 * np.sin(10 * y[i, :, :]) + z[i, :, :] - \
                np.cos(15 * np.multiply(x[i, :, :], y[i, :, :]))
    # y = 1.5 * np.cos(15 * x) + 0.5 * np.sin(10 * y) + z - np.cos(15 * np.multiply(x, y))
    return val
...
# ns = sv.shape[0]
#
# dv = np.arange(0.02, 1.02, 0.02).reshape(-1, 1)
# nd = dv.shape[0]
...
# frames = []
#
# imgs = glob.glob(os.getcwd() + "/fig/path_*.png")
# for i in imgs:
